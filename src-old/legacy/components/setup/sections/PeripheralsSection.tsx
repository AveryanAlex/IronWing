...
export function isPeripheralParamExcluded(paramName: string): boolean {
...
export function PeripheralsSection({ params }: PeripheralsSectionProps) {
    ...
    const additionalGroups = useMemo(() => {
        ...
        for (const name of Object.keys(params.store.params)) {
            if (isPeripheralParamExcluded(name)) continue;
            ...
            <SetupSectionIntro
                ...
                actionSlot={
                    <button
                        ...
                        className={`flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-[11px] font-medium transition-colors ${showConfiguredOnly
