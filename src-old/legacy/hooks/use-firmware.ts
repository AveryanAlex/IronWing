...
export type FirmwareController = FirmwareState & {
  flashSerial: (port: string, baud: number, source: SerialFlashSource, options?: SerialFlashOptions) => Promise<SerialFlowResult>;
  ...
  flashDfuFromOfficialBootloader: (device: DfuDeviceInfo, boardTarget: string) => Promise<DfuRecoveryResult>;
  cancel: () => Promise<void>;
  dismiss: () => void;
  preflight: () => Promise<SerialPreflightInfo>;
  serialReadiness: (request: SerialReadinessRequest) => Promise<SerialReadinessResponse>;
  listPorts: () => Promise<InventoryResult>;
  listDfuDevices: () => Promise<DfuScanResult>;
  catalogTargets: () => Promise<CatalogTargetSummary[]>;
  recoveryCatalogTargets: () => Promise<CatalogTargetSummary[]>;
...
export function useFirmware(): FirmwareController {
  ...
  useEffect(() => {
    firmwareSessionStatus().then(setRawSessionStatus).catch(() => { /* best-effort poll; errors are transient */ });
  }, []);
