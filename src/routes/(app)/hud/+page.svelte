...
import "../../../features/hud/components/hud.css";
import AttitudeBackground from "../../../features/hud/components/AttitudeBackground.svelte";
import ArtificialHorizon from "../../../features/hud/components/ArtificialHorizon.svelte";
import TapeGauge from "../../../features/hud/components/TapeGauge.svelte";
import { createNumberSmoother } from "../../../lib/telemetry-smoothing";
import { getSessionViewStoreContext, getMissionPlannerStoreContext } from "../../../app/shell/runtime-context";
...
let batteryLevel = $derived(
  typeof telemetry.battery_pct === "number" ? Math.max(0, Math.min(100, telemetry.battery_pct)) : 0,
);
</script>
...
<div bind:this={panelRef} class="hud-panel h-full w-full {hasSvs ? 'hud-svs-active' : ''}">
  ...
  {#if hasSvs}
    <div class="hud-svs-bg">
      {#await import("../../../features/hud/components/SvsMap.svelte") then SvsMapModule}
  ...
  {#if !hasSvs && vehiclePosition != null}
    <div class="hud-minimap">
      {#await import("../../../features/hud/components/HudMiniMap.svelte") then HudMiniMapModule}
