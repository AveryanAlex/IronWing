...
function expectHistoryButtonState(
  ...
  expect(button.closest("[data-tooltip-trigger]")).toBeTruthy();
  expect(button.disabled).toBe(disabled);
  // The toolbar is icon-only — count is in aria-label and exposed through the shared tooltip, not textContent.
  return button;
}
...
describe("MissionWorkspace", () => {
  ...
  it("rounds mission coordinate inputs without changing the edited coordinate path", async () => {
    const { plannerStore } = await renderWorkspace({
      setup: ({ plannerStore }) => {
                ...
                command: {
                  Nav: {
                    Waypoint: {
                      position: {
                        RelHome: {
                          latitude_deg: 47.398123456789,
                          longitude_deg: 8.545987654321,
                          relative_alt_m: 25,
                        },
                      },
                      hold_time_s: 0,
    ...
    await waitFor(() => {
      expect((screen.getByTestId(missionWorkspaceTestIds.inspectorLatitude) as HTMLInputElement).value).toBe("47.3981235");
      expect((screen.getByTestId(missionWorkspaceTestIds.inspectorLongitude) as HTMLInputElement).value).toBe("8.5459877");
    });
    ...
    await waitFor(() => {
      const item = get(plannerStore).draftState.active.mission.draftItems[0];
      expect(item?.preview.latitude_deg).toBe(47.4);
      expect(item?.preview.longitude_deg).toBe(8.545987654321);
    });
  });
  ...
  it("enables read and upload for locally-authored drafts after a live vehicle connects", async () => {
    ...
    expect(uploadButton.closest("[data-tooltip-trigger]")).toBeTruthy();
