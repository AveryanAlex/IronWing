...
>
  ...
  {#if model.recoveryText}
    <Alert
      variant="warning"
      title="Metadata recovery is active for Serial Ports."
      testId={setupWorkspaceTestIds.serialPortsRecovery}
    >
  ...
  {#each model.ports.filter(rowRecoveryVisible) as row (row.prefix)}
    <Alert
      variant="warning"
      testId={`${setupWorkspaceTestIds.serialPortsBannerPrefix}-recovery-${row.index}`}
    >
      {row.recoveryText}
    </Alert>
  {/each}
