...
import { Alert, Card, Eyebrow, HelperText, Input, NativeSelect, StagedBadge as SetupStagedBadge } from "../../../components/ui";
...
let boardPresetOptions = $derived([{ value: "", label: "Select board preset" }, ...validBoardPresets.map((preset, index) => ({ value: String(index), label: preset.label }))]);
let sensorPresetOptions = $derived([{ value: "", label: "Select sensor preset" }, ...validSensorPresets.map((preset, index) => ({ value: String(index), label: preset.label }))]);
let chemistryPresetOptions = $derived([{ value: "", label: "Select chemistry" }, ...validChemistries.map((chemistry, index) => ({ value: String(index), label: chemistry.label }))]);
