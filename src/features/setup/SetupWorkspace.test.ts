...
describe("SetupWorkspace", () => {
  ...
  it("stages initial-parameter control previews through the shared review tray", async () => {
    ...
    expect(controlPreview.parentElement?.className).toContain("2xl:grid-cols-2");
    expect(controlPreview.parentElement?.className).not.toContain("xl:grid-cols-3");
    const parameterRow = within(controlPreview).getByText("MOT_THST_EXPO").closest("button");
    expect(parameterRow?.className).toContain("min-w-0");
    const batchCheckbox = within(controlPreview).getByRole("checkbox", { name: "Select" });

    await fireEvent.click(batchCheckbox);
