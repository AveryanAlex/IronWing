...
import { Alert, Badge, EmptyState, FactTile, HelperText } from "../../../components/ui";
...
let filter = $state<ParameterExpertFilter>("all");
...
>
  ...
  {#if view.noticeText}
    <Alert description={view.noticeText} testId={parameterWorkspaceTestIds.notice} variant="warning" />
  {/if}
