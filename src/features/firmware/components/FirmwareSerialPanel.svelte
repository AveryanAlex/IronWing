...
import { Banner, Button, Checkbox, EmptyState, Eyebrow, HelperText, InfoBlock, Input, NativeSelect, Panel, Progress, SectionHeader, SelectableCard, StatusPill } from "../../../components/ui";
    ...
    <Panel padded>
      ...
      <div class="mt-4 grid gap-3 xl:grid-cols-2">
        <Panel padded tone={usingCatalogSource ? "info" : "neutral"} testId={firmwareWorkspaceTestIds.sourceCatalog}>
          <div class="flex flex-wrap items-start justify-between gap-3">
            ...
            {#if !usingCatalogSource}
              <Button size="sm" variant="default" onclick={handleUseCatalogSource}>Use catalog</Button>
            {/if}
          </div>
          ...
          <InfoBlock class="mt-3" title="Target proof">
          ...
          <InfoBlock class="mt-3" title="Catalog source">
          ...
          <Panel padded>
            <div class="flex flex-wrap items-start justify-between gap-3">
              <div>
                <Eyebrow>Manual target override</Eyebrow>
                <HelperText class="mt-1">Search targets when board proof is missing or uncertain.</HelperText>
              </div>
              <Button
                size="sm"
            ...
            {#if manualSectionOpen}
              <div class="mt-3 grid gap-2">
                ...
                <label class="flex min-w-0 flex-col">
                  ...
                  <NativeSelect
                    class="mt-2"
                    testId={firmwareWorkspaceTestIds.manualTargetVehicleFilter}
              ...
              {:else if catalogTargets.length === 0}
                <EmptyState
                  class="mt-3"
                  description="Retry the list or keep browsing with a local APJ."
                  title="No catalog targets are available right now."
                  testId={firmwareWorkspaceTestIds.manualTargetEmpty}
                />
              {:else}
                <EmptyState
                  class="mt-3"
                  description="Clear the search text or switch the vehicle filter to all vehicle types."
                  title="No targets match"
                  testId={firmwareWorkspaceTestIds.manualTargetNoMatches}
                />
              {/if}
          ...
          <Panel padded>
            <div class="flex flex-wrap items-start justify-between gap-3">
              <div>
                <Eyebrow>Official APJ entry</Eyebrow>
                <HelperText class="mt-1">Keep the proven catalog entry selected unless you explicitly need a different release.</HelperText>
              </div>
        ...
        <Panel padded tone={usingLocalSource ? "warning" : "neutral"} testId={firmwareWorkspaceTestIds.sourceLocal}>
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
              <Eyebrow>Advanced source</Eyebrow>
          ...
          <InfoBlock class="mt-3" title="Selected source">
          ...
          {:else if workspaceState.serial.preflightPhase === "ready"}
            <InfoBlock class="mt-3" size="sm" testId={firmwareWorkspaceTestIds.paramBackupState}>
      ...
      <Panel padded testId={firmwareWorkspaceTestIds.serialReadiness}>
        ...
        <HelperText class="mt-2" testId={firmwareWorkspaceTestIds.serialBootloaderTransition}>
          {bootloaderTransitionCopy()}
        </HelperText>

        {#if workspaceState.serial.readiness.response?.validation_pending}
          <HelperText class="mt-2" size="xs" testId={firmwareWorkspaceTestIds.serialValidationPending}>
