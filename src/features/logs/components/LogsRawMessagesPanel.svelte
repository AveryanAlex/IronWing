...
<Panel testId="logs-raw-messages-panel">
  <div class="flex flex-col gap-3">
    <div class="flex items-start justify-between gap-3 max-lg:flex-col">
      <div>
        <Eyebrow>Forensic browser</Eyebrow>
        <h3 class={titleClass}>Raw messages</h3>
        <HelperText>Filter indexed records, inspect message payloads, and export the current filtered view as CSV.</HelperText>
      </div>
    ...
    {:else}
      <div class="flex flex-wrap gap-3">
        <label class="flex-[1_1_10rem]">
          <Eyebrow as="span" class="mb-1 block">Message types</Eyebrow>
          <Input testId="logs-raw-type-filter" oninput={(event) => updateFilters((current) => ({ ...current, messageTypesInput: (event.currentTarget as HTMLInputElement).value }))} value={draftFilters.messageTypesInput} />
        </label>
        <label class="flex-[1_1_10rem]">
          <Eyebrow as="span" class="mb-1 block">Text</Eyebrow>
          <Input testId="logs-raw-text-filter" oninput={(event) => updateFilters((current) => ({ ...current, textInput: (event.currentTarget as HTMLInputElement).value }))} value={draftFilters.textInput} />
        </label>
        <label class="flex-[1_1_10rem]">
          <Eyebrow as="span" class="mb-1 block">Start μs</Eyebrow>
          <Input testId="logs-raw-start-filter" oninput={(event) => updateFilters((current) => ({ ...current, startUsecInput: (event.currentTarget as HTMLInputElement).value }))} value={draftFilters.startUsecInput} />
          {#if numericValidation.startUsec}
            <HelperText class="mt-1" size="xs" tone="danger" testId="logs-raw-start-error">{numericValidation.startUsec}</HelperText>
          {/if}
        </label>
        <label class="flex-[1_1_10rem]">
          <Eyebrow as="span" class="mb-1 block">End μs</Eyebrow>
          <Input testId="logs-raw-end-filter" oninput={(event) => updateFilters((current) => ({ ...current, endUsecInput: (event.currentTarget as HTMLInputElement).value }))} value={draftFilters.endUsecInput} />
          {#if numericValidation.endUsec}
            <HelperText class="mt-1" size="xs" tone="danger" testId="logs-raw-end-error">{numericValidation.endUsec}</HelperText>
          {/if}
        </label>
        <label class="flex-[1_1_10rem]">
          <Eyebrow as="span" class="mb-1 block">Page size</Eyebrow>
          <Input testId="logs-raw-limit-filter" max={MAX_RAW_PAGE_SIZE} min="1" oninput={(event) => updateFilters((current) => ({ ...current, limitInput: (event.currentTarget as HTMLInputElement).value }))} step="1" type="number" value={draftFilters.limitInput} />
          {#if numericValidation.limit}
            <HelperText class="mt-1" size="xs" tone="danger" testId="logs-raw-limit-error">{numericValidation.limit}</HelperText>
          {/if}
        </label>
        <Checkbox
          checked={draftFilters.includeDetail}
          label="Detail"
          onCheckedChange={(checked) => updateFilters((current) => ({ ...current, includeDetail: checked }))}
        />
        <Checkbox
          checked={draftFilters.includeHex}
          label="Hex"
          onCheckedChange={(checked) => updateFilters((current) => ({ ...current, includeHex: checked }))}
        />
      </div>
      ...
      <div class="flex flex-col gap-3">
        {#each draftFilters.fieldFilters as fieldFilter, index (fieldFilter.id)}
          <div class="flex flex-1 gap-3 max-md:flex-col">
            <Input aria-label={`Field name ${index + 1}`} class="flex-1" testId={`logs-raw-field-name-${index}`} oninput={(event) => updateFieldFilter(fieldFilter.id, { field: (event.currentTarget as HTMLInputElement).value })} placeholder="field" value={fieldFilter.field} />
            <Input aria-label={`Field value ${index + 1}`} class="flex-1" testId={`logs-raw-field-value-${index}`} oninput={(event) => updateFieldFilter(fieldFilter.id, { value_text: (event.currentTarget as HTMLInputElement).value })} placeholder="contains" value={fieldFilter.value_text ?? ""} />
      ...
      <div class="flex items-start justify-between gap-3 max-lg:flex-col">
        <div class="flex flex-wrap gap-3">
          <Button variant="default" testId="logs-raw-run-query" onclick={runFirstPageQuery}>Run query</Button>
      ...
      <div class="flex flex-1 gap-3 max-lg:w-full">
        <Input aria-label="Export destination path" class="flex-1" testId="logs-raw-export-destination" oninput={(event) => (exportDestination = (event.currentTarget as HTMLInputElement).value)} placeholder="/tmp/raw-messages.csv" value={exportDestination} />
        <Button
      ...
      <aside class="flex min-h-0 flex-col gap-3 overflow-auto rounded-lg border border-border bg-bg-primary p-3">
        ...
        {#if selectedRecord}
          ...
          {#if selectedRecord.detail !== null}
            <Eyebrow as="span" class="mb-1 block">Raw detail</Eyebrow>
            <pre>{detailJson(selectedRecord.detail)}</pre>
          {/if}
          {#if selectedRecord.hex_payload}
            <Eyebrow as="span" class="mb-1 block">Hex payload</Eyebrow>
            <pre>{selectedRecord.hex_payload}</pre>
          {/if}
        {:else}
          <HelperText>Run a query and choose a row to inspect raw fields and payload details.</HelperText>
        {/if}
      </aside>
    </div>
  {/if}
