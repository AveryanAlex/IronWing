...
#[derive(Debug, Clone)]
pub struct ExternalFlashPayload {
...
pub fn parse_apj(data: &[u8]) -> Result<SerialArtifact, FirmwareError> {
    let apj: ApjFile =
...
pub fn validate_recovery_bin(data: &[u8]) -> Result<DfuRecoveryArtifact, FirmwareError> {
    if data.is_empty() {
        return Err(FirmwareError::ArtifactInvalid {
...
pub fn classify_artifact(path: &str) -> Result<ArtifactClassification, FirmwareError> {
...
pub fn validate_recovery_file_type(path: &str) -> Result<(), FirmwareError> {
