...
impl SessionRuntime {
    pub const DEFAULT_PENDING_SESSION_TTL: Duration = Duration::from_secs(2);

    pub fn new() -> Self {
        Self::with_pending_ttl(Self::DEFAULT_PENDING_SESSION_TTL)
    }

    pub fn with_pending_ttl(pending_ttl: Duration) -> Self {
        Self {
            live_active: None,
    ...
    pub fn open_session_snapshot(&mut self, source_kind: SourceKind) -> OpenSessionSnapshot {
        ...
        let snapshot = OpenSessionSnapshot {
            envelope,
            session: DomainProvenance::Bootstrap.present_session(),
            telemetry: crate::ipc::DomainValue::missing(DomainProvenance::Bootstrap),
            mission_state: None,
            param_store: None,
            param_progress: None,
    ...
    pub fn issue_playback_seek(&mut self) -> Result<SessionEnvelope, OperationFailure> {
        let Some(active) = self.playback_active.as_mut() else {
            return Err(playback_session_inactive_failure(OperationId::ReplaySeek));
        };

        if active.source_kind != SourceKind::Playback {
            return Err(OperationFailure {
                operation_id: OperationId::ReplaySeek,
                reason: Reason {
                    kind: ReasonKind::Conflict,
    ...
    pub fn active_playback_envelope(
        &self,
    ...
    pub const fn effective_source_kind(&self) -> SourceKind {
    ...
    pub fn sweep_expired_pending(&mut self, now: Instant) {
...
impl BootstrapSessionDomain for DomainProvenance {
    fn present_session(self) -> crate::ipc::DomainValue<SessionSnapshot> {
        crate::ipc::DomainValue::present(
            SessionSnapshot {
...
fn playback_session_inactive_failure(operation_id: OperationId) -> OperationFailure {
    OperationFailure {
        operation_id,
        reason: Reason {
            kind: ReasonKind::Conflict,
