...
const barometerPrimaryValues = [
  { code: 0, label: "First barometer" },
  { code: 1, label: "Second barometer" },
] satisfies SetupMetadataOption[];
const ahrsEkfTypeValues = [
  { code: 2, label: "EKF2" },
  { code: 3, label: "EKF3" },
] satisfies SetupMetadataOption[];
const ekfSourceValues = [
  { code: 0, label: "None" },
  { code: 1, label: "Wheel encoder" },
  { code: 2, label: "GNSS" },
  { code: 3, label: "Beacon" },
  { code: 4, label: "Optical flow" },
  { code: 6, label: "External nav" },
  { code: 7, label: "Wheel odometry" },
] satisfies SetupMetadataOption[];
const ekfVerticalSourceValues = [
  { code: 0, label: "None" },
  { code: 1, label: "Barometer" },
  { code: 2, label: "Rangefinder" },
  { code: 3, label: "GNSS" },
  { code: 6, label: "External nav" },
] satisfies SetupMetadataOption[];
const ekfYawSourceValues = [
  { code: 0, label: "None" },
  { code: 1, label: "Compass" },
  { code: 2, label: "GNSS" },
  { code: 6, label: "External nav" },
] satisfies SetupMetadataOption[];
...
function createCommonSetupMetadataParams(modeValues: SetupMetadataOption[]): SetupMetadataParam[] {
  return [
    ...
    enumParam("GPS1_TYPE", "GNSS 1 type", "Primary GNSS receiver type.", gpsTypeValues),
    enumParam("GPS_TYPE", "GNSS type", "Legacy primary GNSS receiver type.", gpsTypeValues),
    enumParam("GPS2_TYPE", "GNSS 2 type", "Secondary GNSS receiver type.", gpsTypeValues),
    enumParam("GPS_AUTO_SWITCH", "GNSS auto switch", "How the autopilot switches between GNSS receivers.", gpsAutoSwitchValues),
    enumParam("GPS_AUTO_CONFIG", "GNSS auto config", "Automatically configure the attached GNSS receiver on boot.", enabledDisabledValues),
    ...
    enumParam("COMPASS_USE2", "Compass 2 use", "Use the second compass for navigation.", enabledDisabledValues),
    enumParam("COMPASS_USE3", "Compass 3 use", "Use the third compass for navigation.", enabledDisabledValues),
    enumParam("COMPASS_AUTO_ROT", "Compass auto rotation", "Automatically resolve compass orientation when supported.", enabledDisabledValues),
    enumParam("COMPASS_ORIENT", "Compass orientation", "Manual orientation for the primary compass.", orientationValues, { rebootRequired: true }),
    numericParam("COMPASS_DEV_ID", "Compass device", "Primary compass device id."),
    enumParam("BARO_PRIMARY", "Primary barometer", "Primary barometer selection.", barometerPrimaryValues),
    numericParam("BARO_ALT_OFFSET", "Altitude offset", "Primary barometer altitude offset.", { unitText: "m" }),
    numericParam("BARO1_GND_PRESS", "Ground pressure", "Primary barometer ground pressure reference.", { unitText: "Pa" }),
    numericParam("BARO_GND_TEMP", "Ground temperature", "Primary barometer ground temperature reference.", { unitText: "°C" }),
    numericParam("BARO_ALTERR_MAX", "Altitude discrepancy limit", "Maximum allowed GNSS and barometer altitude difference for arming checks.", { unitText: "m" }),
    enumParam("AHRS_EKF_TYPE", "AHRS estimator", "Estimator family used for navigation and attitude calculations.", ahrsEkfTypeValues),
    enumParam("AHRS_GPS_USE", "AHRS GNSS use", "Use GNSS data in the AHRS navigation solution.", enabledDisabledValues),
    enumParam("EK3_SRC1_POSXY", "EKF horizontal position source", "Primary EKF horizontal position source.", ekfSourceValues),
    enumParam("EK3_SRC1_VELXY", "EKF horizontal velocity source", "Primary EKF horizontal velocity source.", ekfSourceValues),
    enumParam("EK3_SRC1_POSZ", "EKF vertical position source", "Primary EKF vertical position source.", ekfVerticalSourceValues),
    enumParam("EK3_SRC1_VELZ", "EKF vertical velocity source", "Primary EKF vertical velocity source.", ekfVerticalSourceValues),
    enumParam("EK3_SRC1_YAW", "EKF yaw source", "Primary EKF yaw source.", ekfYawSourceValues),
    numericParam("WPNAV_SPEED", "Waypoint speed", "Target horizontal speed for waypoint navigation.", { unitText: "cm/s" }),
    numericParam("WPNAV_ACCEL", "Waypoint acceleration", "Horizontal acceleration limit for waypoint navigation.", { unitText: "cm/s²" }),
    numericParam("WPNAV_SPEED_UP", "Waypoint climb speed", "Vertical climb speed limit for waypoint navigation.", { unitText: "cm/s" }),
    numericParam("WPNAV_SPEED_DN", "Waypoint descent speed", "Vertical descent speed limit for waypoint navigation.", { unitText: "cm/s" }),
...
const setupParamStoreEntries: SetupParamSeed[] = [
  ...
  { name: "COMPASS_USE2", value: 0 },
  { name: "COMPASS_USE3", value: 0 },
  { name: "COMPASS_AUTO_ROT", value: 1 },
  { name: "COMPASS_ORIENT", value: 0 },
  { name: "COMPASS_DEV_ID", value: 12345 },
  { name: "BARO_PRIMARY", value: 0 },
  { name: "BARO_ALT_OFFSET", value: 0 },
  { name: "BARO1_GND_PRESS", value: 101325 },
  { name: "BARO_GND_TEMP", value: 25 },
  { name: "BARO_ALTERR_MAX", value: 2000 },
  { name: "AHRS_EKF_TYPE", value: 3 },
  { name: "AHRS_GPS_USE", value: 1 },
  { name: "EK3_SRC1_POSXY", value: 3 },
  { name: "EK3_SRC1_VELXY", value: 3 },
  { name: "EK3_SRC1_POSZ", value: 1 },
  { name: "EK3_SRC1_VELZ", value: 0 },
  { name: "EK3_SRC1_YAW", value: 1 },
  { name: "WPNAV_SPEED", value: 1000 },
  { name: "WPNAV_ACCEL", value: 250 },
  { name: "WPNAV_SPEED_UP", value: 250 },
  { name: "WPNAV_SPEED_DN", value: 150 },
